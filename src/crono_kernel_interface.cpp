...
CRONO_KERNEL_PciScanDevices(uint32_t dwVendorId, uint32_t dwDeviceId,
  ...
  while ((en = readdir(dr)) != NULL) {
    ...
    crono_read_vendor_device(domain, bus, dev, func, (unsigned int *)&vendor_id,
                             (unsigned int *)&device_id);

    // Check values & fill pPciScanResult if device matches the
    // Vendor/Device
    if (((vendor_id == dwVendorId) || (((uint32_t)PCI_ANY_ID) == dwVendorId)) &&
        ((device_id == dwDeviceId) || (((uint32_t)PCI_ANY_ID) == dwDeviceId))) {
  ...
  *phDev = NULL;
  ...
  while ((en = readdir(dr)) != NULL) {
    ...
    if ((pDeviceInfo->pciSlot.dwDomain == domain) &&
      ...
      pDevice->pciSlot.dwDomain = pDeviceInfo->pciSlot.dwDomain;
      pDevice->pciSlot.dwBus = pDeviceInfo->pciSlot.dwBus;
      pDevice->pciSlot.dwSlot = pDeviceInfo->pciSlot.dwSlot;
      pDevice->pciSlot.dwFunction = pDeviceInfo->pciSlot.dwFunction;

      // Get device `Vendor ID` and `Device ID` and set them
      // to `pDevice`
      unsigned int vendor_id, device_id;
      err = crono_read_vendor_device(domain, bus, dev, func, &vendor_id,
                                     &device_id);
      ...
      pciaddr_t dwSize = 0;
      err = crono_get_BAR0_mem_addr(domain, bus, dev, func, 0, &dwSize,
                                    &BAR_base_mem_address, NULL);
      if (CRONO_SUCCESS != err) {
        goto device_error;
      }
      pDevice->bar_addr.pUserDirectMemAddr = (size_t)BAR_base_mem_address;
...
uint32_t CRONO_KERNEL_CardCleanupSetup(CRONO_KERNEL_DEVICE_HANDLE hDev,
                                       CRONO_KERNEL_CMD *Cmd, uint32_t dwCmds,
                                       int bForceCleanup) {
  char miscdev_path[PATH_MAX];
  int miscdev_fd;
  ...
  if (CRONO_SUCCESS != ioctl(miscdev_fd, IOCTL_CRONO_UNLOCK_BUFFER, NULL)) {
    return -EPERM;
  }
...
uint32_t CRONO_KERNEL_PciReadCfg32(CRONO_KERNEL_DEVICE_HANDLE hDev,
  ...
  err = crono_get_config_space_size(
      ...
      pDevice->pciSlot.dwSlot, pDevice->pciSlot.dwFunction, &config_space_size);
  if (CRONO_SUCCESS != err) {
    return err;
  }
  ...
  err = crono_read_config(pDevice->pciSlot.dwDomain, pDevice->pciSlot.dwBus,
...
uint32_t CRONO_KERNEL_PciWriteCfg32(CRONO_KERNEL_DEVICE_HANDLE hDev,
  ...
  err = crono_write_config(pDevice->pciSlot.dwDomain, pDevice->pciSlot.dwBus,
                           pDevice->pciSlot.dwSlot, pDevice->pciSlot.dwFunction,
                           &val, dwOffset, sizeof(val), &bytes_written);
...
uint32_t CRONO_KERNEL_ReadAddr8(CRONO_KERNEL_DEVICE_HANDLE hDev,
                                uint32_t dwAddrSpace, uint32_t dwOffset,
                                unsigned char *val) {
  // Init variables and validate parameters
  CRONO_INIT_HDEV_FUNC(hDev);
...
uint32_t CRONO_KERNEL_ReadAddr32(CRONO_KERNEL_DEVICE_HANDLE hDev,
  ...
  *val = *((volatile uint32_t *)(((unsigned char *)(pDevice->bar_addr
                                                        .pUserDirectMemAddr)) +
                                 dwOffset));
...
uint32_t CRONO_KERNEL_ReadAddr64(CRONO_KERNEL_DEVICE_HANDLE hDev,
  ...
  // Copy memory
  *val = *((volatile uint64_t *)(((unsigned char *)(pDevice->bar_addr
                                                        .pUserDirectMemAddr)) +
                                 dwOffset));
...
uint32_t CRONO_KERNEL_WriteAddr8(CRONO_KERNEL_DEVICE_HANDLE hDev,
  ...
  // Copy memory
  *((volatile unsigned char *)(((unsigned char *)(pDevice->bar_addr
                                                      .pUserDirectMemAddr)) +
                               dwOffset)) = val;
...
uint32_t CRONO_KERNEL_WriteAddr16(CRONO_KERNEL_DEVICE_HANDLE hDev,
  ...
  // Copy memory
  *((volatile unsigned short *)(((unsigned char *)(pDevice->bar_addr
                                                       .pUserDirectMemAddr)) +
                                dwOffset)) = val;
...
#define DIV_ROUND_UP(n, d) (((n) + (d)-1) / (d))
  ...
  CRONO_DEBUG("Allocating memory of size = <%lu>, of pages count <%d>\n",
              sizeof(CRONO_KERNEL_DMA_PAGE) * pDma->dwPages, pDma->dwPages);
  pDma->Page = (CRONO_KERNEL_DMA_PAGE *)malloc(sizeof(CRONO_KERNEL_DMA_PAGE) *
                                               pDma->dwPages);
  if (NULL == pDma->Page) {
  ...
  *ppDma = pDma; // Do not ppDma = &pDma
  ...
  for (unsigned int ipage = 0; ipage < (pDma->dwPages < 5 ? pDma->dwPages : 5);
       ipage++) {
    fprintf(stdout, "Buffer Page <%d> Physical Address is <%p>\n", ipage,
            (void *)(pDma->Page[ipage].pPhysicalAddr));
  }
#endif
...
uint32_t CRONO_KERNEL_DMASGBufUnlock(CRONO_KERNEL_DEVICE_HANDLE hDev,
  ...
  params.ppDma = NULL;
  miscdev_fd = 0;
  params.ppDma = (CRONO_KERNEL_DMA_SG **)malloc(sizeof(CRONO_KERNEL_DMA_SG **));
  params.ppDma[0] = pDma;
  params.ulpDma = (unsigned long)pDma;
